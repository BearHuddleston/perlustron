...
expect(app.includes("function renderForensicVerdictCard"), "Summary should render an answer-first Forensic Verdict card.");
expect(app.includes('modeCard("Forensic Verdict"'), "Forensic Verdict should have a clear top-card heading.");
expect(app.includes("function forensicVerdictOutcome"), "Forensic Verdict should summarize the run outcome explicitly.");
expect(app.includes("function forensicVerdictFirstCriticalEvent"), "Forensic Verdict should call out the first critical or suspicious event.");
expect(app.includes("Highest-confidence finding"), "Forensic Verdict should name the highest-priority finding before the detail queue.");
expect(app.includes('modeButton("Inspect Highest-Priority Finding"'), "Forensic Verdict should expose one obvious primary CTA for the top finding.");
const confidenceRankBlock = app.match(/function insightConfidenceRank[\s\S]*?\n}\n\nfunction forensicVerdictFinding/)?.[0] ?? "";
...
expect(app.includes('modeButton("View Evidence"'), "Summary insights should expose one primary View Evidence CTA per finding.");
expect(!app.includes('modeButton("Timeline Evidence"') && !app.includes('modeButton("Transcript Evidence"') && !app.includes('modeButton("Raw Evidence"'), "Summary insights should not render repeated per-finding evidence CTAs.");
expect(app.includes("function summaryInsightEvidenceDrawer"), "Summary insights should render a unified evidence drawer.");
expect(app.includes('trigger.setAttribute("aria-controls", drawerId)') && app.includes('trigger.setAttribute("aria-expanded", "false")'), "Summary evidence drawer trigger should expose keyboard and screen-reader expansion state.");
expect(app.includes('surfaceActions.setAttribute("role", "group")'), "Summary evidence drawer surfaces should be grouped for assistive tech.");
expect(app.includes("function openInsightEvidence"), "Summary drawer actions should route through an explicit insight evidence helper.");
