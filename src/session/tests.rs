...
fn scan_and_sanitize_redact_common_secrets() {
    let input = temp_jsonl_path("secret-scan-input");
    ...
    assert!(!sanitized.contains("sk-abc...wxyz"));
...
fn adversarial_redaction_scan_and_standard_sanitize_redact_known_secret_shapes() {
    let input = fixture_path("adversarial-redaction-codex.jsonl");
...
fn adversarial_redaction_strict_exports_do_not_leak_codex_fixture_secrets() {
    let fixture = fixture_path("adversarial-redaction-codex.jsonl");
...
fn adversarial_redaction_strict_exports_do_not_leak_claude_image_or_path_metadata() {
    let fixture = fixture_path("adversarial-redaction-claude.jsonl");
...
fn adversarial_redaction_fixtures_parse() {
    let cache = Mutex::new(HashMap::new());
    let codex = load_session_graph(
        SessionSource::Codex,
        &fixture_path("adversarial-redaction-codex.jsonl"),
        &cache,
    )
    .unwrap();
    assert!(!codex.prompts.is_empty());
    assert!(codex.prompts.iter().any(|prompt| !prompt.calls.is_empty()));
    assert!(
        codex
            .prompts
            .iter()
    ...
    assert!(codex.parser_health.unknown_event_count >= 1);
    assert!(codex.parser_health.malformed_line_count >= 1);
    assert!(codex.parser_health.image_count >= 1);

    let claude = load_session_graph(
        SessionSource::Claude,
        &fixture_path("adversarial-redaction-claude.jsonl"),
        &cache,
    )
    .unwrap();
    assert!(claude.parser_health.image_count >= 1);
    assert!(claude.parser_health.tool_result_count >= 1);
}
...
fn scenario_fixtures_cover_debugging_workflows() {
    let scenarios = [
        ...
        ("adversarial-redaction-codex.jsonl", SessionSource::Codex),
        ("adversarial-redaction-claude.jsonl", SessionSource::Claude),
